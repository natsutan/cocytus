...
int CQT_Conv2D_same_3x3_ifx16_wfx16_wfx16_ofx16 (CQT_LAYER *lp, void *inp, void *outp)
{
    ...
    FIXP16 *ip = (FIXP16 *)inp;  //Qpos:input_q
    FIXP16 *op = outp;               //Qpos:output_q
    FIXP16 *wp = cnvp->weight_p;     //Qpos:weight_q
    FIXP16 *bp = cnvp->bias_p;       //Qpos:weight_q
    ...
    int f, x, y, n;
    int idx_i,idx_o;
    FIXP16 w_data;                 //Qpos:weight_q
    FIXP16 o_data;                 //Qpos:output_q
    ...
    max = SHRT_MAX;
    min = SHRT_MIN;
    memset(op, 0.0, fill_num * input_size_y * input_size_x * sizeof(FIXP16));
